...
| `requirements_current` | `id`, `tenant_id`, `session_id`, `field`, typed value columns (`value_integer`, `value_text`, `value_text_array`, `value_numeric`, `currency`), `confirmed_at`, `source_turn_id`, timestamps/version | unique `(tenant_id,session_id,field)`; exactly one value representation allowed | Deleted with session after history retention |
| `requirement_changes` | `id`, `tenant_id`, `session_id`, `requirement_id`, `field`, typed old/new values, `confirmed`, `source_turn_id`, `event_id`, `changed_at` | unique `event_id`; index `(tenant_id,session_id,changed_at,id)` | Immutable; same retention as durable events |
