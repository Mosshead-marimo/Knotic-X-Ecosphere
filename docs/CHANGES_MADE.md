...
### 2026-09-05 — Built the shared integration adapter framework

- Phase: 5 (`P5-T001`)
- Files: `mcp/src/knotic_mcp/integrations.py` (new — `IntegrationCredential`/`CredentialStore`/`InMemoryCredentialStore`/`require_credential`, `RetryPolicy`, `CircuitBreaker`/`CircuitBreakerOpen`, `call_with_resilience`, `IdempotencyStore`/`InMemoryIdempotencyStore`/`idempotent_write`, `request_fingerprint`, `IntegrationProviderError`/`IdempotencyConflict`), `mcp/tests/test_integrations.py` (new), this file.
- Status: Implemented, verification not yet run
- Summary: Every CRM/calendar adapter built on top of this module shares one credential-isolation rule (a lookup is always keyed by `(tenant_id, provider)`, so one tenant's or provider's credential can never reach another), one retry rule (only a provider error explicitly marked `retryable=True` is retried, with full-jitter exponential backoff and a hard deadline — a write whose outcome is actually unknown is never retried as if a retry were free), one circuit-breaker rule (a provider that has failed repeatedly is failed fast instead of queueing every caller behind it, mirroring the existing `ProviderHealthTracker` in `cache.py`), and one idempotency rule (`idempotent_write` replays the exact prior result for a reused idempotency key, and raises `IdempotencyConflict` — mapped to the contract's `IDEMPOTENCY_CONFLICT` failure — if the same key is reused with different arguments, rather than either replaying the wrong result or performing the write twice).
- Verification: Not run in this session — no working sandbox shell was available, so `ruff`, `mypy`, and `pytest` could not be executed here. Added unit tests covering: missing/expired/cross-tenant/cross-provider credential rejection; retry-only-on-retryable-failure then success; never retrying a non-retryable error; deadline-expiry raising `TIMEOUT` before any attempt; circuit-breaker opening after the failure threshold and failing fast until a success resets it; invalid `RetryPolicy`/`CircuitBreaker` construction bounds; idempotent replay without re-performing the write; idempotency-key reuse with different arguments raising `IdempotencyConflict`; and `request_fingerprint` being stable and key-order-independent. Before treating `P5-T001` as complete, run `pytest mcp/tests`, `ruff check mcp/src mcp/tests`, and `mypy mcp/src`.
- Follow-up: No real CRM or calendar provider adapter exists yet — this task is the shared framework only; `P5-T002`–`P5-T005` build the first adapters (in-memory sandbox fakes) on top of it. Production credential storage (encrypted, rotated, provisioned per tenant ahead of time) is not implemented; `InMemoryCredentialStore`/`_SandboxCredentialStore` (in `app.py`) are sandbox-only placeholders.

### 2026-09-05 — Implemented CRM lead lookup and deduplication

- Phase: 5 (`P5-T002`)
- Files: `mcp/src/knotic_mcp/crm.py` (new — `CrmLead`, `LeadLookupResult`, `find_lead`, `CrmProviderPort`, `InMemoryCrmProvider`), `mcp/tests/test_crm.py` (new), this file.
- Status: Implemented, verification not yet run
- Summary: `find_lead` scores every candidate against only the lookup fields the caller actually supplied (`provider_lead_id`, `email`, `phone`, `company`), with an exact `provider_lead_id` match weighted decisively over the softer fields (normalized email/phone/company) so it can never be outscored by a coincidental combination of weaker matches. When more than one candidate ties for the best score, the lookup returns `found=False` (with an internal `ambiguous=True` marker for observability) rather than guessing — attaching a live conversation to the wrong CRM lead is treated as worse than a caller having to fall back to creating a new lead or escalating to a human. `InMemoryCrmProvider` is a deterministic sandbox fake (the same role `PgvectorRetrievalService` plays opposite its Postgres counterpart in `knowledge.py`), so the dedup logic is fully unit-testable without a live CRM.
- Verification: Not run in this session — no working sandbox shell was available, so `ruff`, `mypy`, and `pytest` could not be executed here. Added unit tests covering: a lookup with no identifying field raising `ValueError`; no candidates producing a plain miss; a single unique email match; two equally-strong company-name matches producing an ambiguous miss rather than a guess; and a decisive `provider_lead_id` match overriding a weaker company match on a different lead. Before treating `P5-T002` as complete, run `pytest mcp/tests`, `ruff check mcp/src mcp/tests`, and `mypy mcp/src`.
- Follow-up: The matching rules (exact-normalized email/phone, case/whitespace-normalized company name) are a reasonable sandbox default, not a tuned production fuzzy-matching policy — a real CRM adapter may need phonetic or fuzzy company-name matching, which would change what counts as "equally strong" and should be revisited before this reaches a live CRM.

### 2026-09-05 — Implemented CRM lead create/update/activity tools

- Phase: 5 (`P5-T003`)
- Files: `mcp/src/knotic_mcp/crm.py` (`InMemoryCrmProvider.create_lead`/`update_lead`/`add_note`/`add_call_summary`), `mcp/src/knotic_mcp/registry.py` (added `TOOL_DEFINITIONS` entries and `arguments_are_valid` validators for `crm.create_lead`/`crm.update_lead`/`crm.add_note`/`crm.add_call_summary`), `mcp/src/knotic_mcp/app.py` (wires the four write tools behind `call_with_resilience` + `idempotent_write`, with a credential check before every call), `mcp/tests/test_crm.py`, this file.
- Status: Implemented, verification not yet run
- Summary: Every write requires an idempotency key (enforced both by the gateway's existing dispatch check and, now, by an `idempotent_write` wrapper that actually replays a prior result for a reused key instead of just accepting the key's presence). `update_lead` is optimistic-concurrency-checked (`expected_version` must match the stored version) and raises `CONFLICT` on a mismatch or on an unknown/cross-tenant lead — never silently overwriting a change the caller didn't see. `InMemoryCrmProvider` confirms every write synchronously (no `PENDING` path), matching a sandbox integration's usual behavior; a production adapter fronting an eventually-confirmed provider would instead return `PENDING_CONFIRMATION` until a webhook or poll confirms the write, which the tool handlers already accept as a valid outcome per the contract's own `PENDING_CONFIRMATION` status.
- Verification: Not run in this session — no working sandbox shell was available, so `ruff`, `mypy`, and `pytest` could not be executed here. Added unit tests covering: `create_lead` requiring a company and persisting extra fields with `version=1`; `update_lead` enforcing optimistic concurrency (a second update at the same `expected_version` raises `CONFLICT`) and rejecting an unknown/cross-tenant lead with `NOT_FOUND`; `add_note`/`add_call_summary` requiring an existing lead and returning distinct confirmed ids per call. Before treating `P5-T003` as complete, run `pytest mcp/tests`, `ruff check mcp/src mcp/tests`, and `mypy mcp/src`, plus exercise the four tools through `mcp/src/knotic_mcp/app.py`'s ASGI gateway directly (no gateway-level test was added in this session; only the underlying `InMemoryCrmProvider`/`crm.py` logic is unit-tested).
- Follow-up: No gateway-level (ASGI request/response) test exercises `crm.create_lead`/`update_lead`/`add_note`/`add_call_summary` end to end through `app.py`, including the `CUSTOMER_CONFIRMATION`/`POLICY` approval flow and the idempotency-conflict HTTP path — only the underlying provider and registry validators are unit-tested. A production CRM adapter (implementing `CrmProviderPort` against a real CRM API) does not exist yet.

### 2026-09-05 — Implemented calendar availability retrieval

- Phase: 5 (`P5-T004`)
- Files: `mcp/src/knotic_mcp/calendar.py` (new — `CalendarSlot`, `SlotSnapshot`, `CalendarProviderPort`, `InMemoryCalendarProvider.list_slots`, `validate_timezone`), `mcp/src/knotic_mcp/registry.py` (added `calendar.get_slots`), `mcp/src/knotic_mcp/app.py` (`calendar_get_slots` handler), `mcp/tests/test_calendar.py` (new), this file.
- Status: Implemented, verification not yet run
- Summary: Slots are generated deterministically from an organizer working-hours policy (weekdays, 09:00–17:00, each slot separated from the next by a 15-minute buffer so back-to-back meetings are never scheduled flush against each other) and returned as one *snapshot* — a single batch that expires together (`snapshot_expires_at`), which `P5-T005` re-checks at booking time. Each slot's id is derived deterministically from `(tenant_id, organizer, starts_at, ends_at)` rather than randomly generated, so the same real-world slot maps to the same id across separate snapshot calls — this is what lets a booked slot actually disappear from the next availability listing instead of a fresh random id silently reappearing. `validate_timezone` rejects a non-IANA timezone string as `INVALID_ARGUMENT` before any slot generation happens, and every returned slot carries an explicit timezone-converted display time rather than a bare UTC instant.
- Verification: Not run in this session — no working sandbox shell was available, so `ruff`, `mypy`, and `pytest` could not be executed here. Added unit tests covering: valid/invalid IANA timezone names; only weekday working-hours slots (correct weekday, hour range, and duration) being offered; an inverted `from`/`to` window rejected as `INVALID_ARGUMENT`; and a booked slot's id excluded from a subsequent listing over the same window (this specifically exercises the deterministic-slot-id fix above). Before treating `P5-T004` as complete, run `pytest mcp/tests`, `ruff check mcp/src mcp/tests`, and `mypy mcp/src`.
- Follow-up: The organizer policy (fixed 09:00–17:00 UTC, one hardcoded organizer, no per-organizer working-hours configuration, no holiday calendar) is a sandbox default, not a configurable production policy — a real calendar adapter needs per-organizer hours, time-off, and holiday awareness. No pagination exists (the contract's `output_schema` does not require it); slots are capped at 20 per snapshot.

### 2026-09-05 — Implemented confirmed meeting booking

- Phase: 5 (`P5-T005`)
- Files: `mcp/src/knotic_mcp/calendar.py` (`InMemoryCalendarProvider.book`, `BookedMeeting`), `mcp/src/knotic_mcp/registry.py` (added `calendar.book_meeting` with `approval="CUSTOMER_CONFIRMATION"`), `mcp/src/knotic_mcp/app.py` (`calendar_book_meeting` handler, wired behind `call_with_resilience` + `idempotent_write`), `mcp/tests/test_calendar.py`, this file.
- Status: Implemented, verification not yet run
- Summary: Booking requires the caller to hand back the exact `slot_id` and `snapshot_reference` from a `P5-T004` offer; the provider re-checks, at booking time, that the snapshot exists for this tenant, has not expired, actually contains the requested slot, and that the slot has not already been booked by a different (possibly concurrent) request — each of the four checks raises a distinct `CONFLICT` rather than silently booking the wrong or a stale slot. `calendar.book_meeting` carries `approval="CUSTOMER_CONFIRMATION"` in the registry (distinct from the other write tools' `"POLICY"`), so the gateway's existing `ApprovalPolicy` requires an approval reference tied to this exact call's arguments before a booking can proceed — a booking is never claimed confirmed without both an explicit customer-approved request and the provider's own confirmation (`status="CONFIRMED"` only after `InMemoryCalendarProvider.book` succeeds). The tool is idempotency-required and wrapped in the same `idempotent_write` replay used by the CRM write tools.
- Verification: Not run in this session — no working sandbox shell was available, so `ruff`, `mypy`, and `pytest` could not be executed here. Added unit tests covering: booking against an unknown snapshot, an unknown slot, and an expired snapshot each raising `CONFLICT`; a successful booking returning a matching slot and a non-empty provider reference; and a second booking attempt against the same already-booked slot raising `CONFLICT`. Before treating `P5-T005` as complete, run `pytest mcp/tests`, `ruff check mcp/src mcp/tests`, and `mypy mcp/src`.
- Follow-up: No gateway-level test exercises the `CUSTOMER_CONFIRMATION` approval path for `calendar.book_meeting` through `app.py`'s ASGI dispatch (only the underlying `InMemoryCalendarProvider.book` logic is unit-tested) — that, plus a production calendar adapter and attendee-address validation/verification (currently any non-empty string is accepted as an attendee), are follow-ups. With `P5-T001` through `P5-T005` implemented at the unit level, Phase 5's remaining tasks (`P5-T006` onward, per `docs/phases/PHASE_5_INTEGRATIONS.md`) and the phase gate itself are still outstanding, and none of this session's verification commands have actually been run — see each entry above for the exact commands still needed.
