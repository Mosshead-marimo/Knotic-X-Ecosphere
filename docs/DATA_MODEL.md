...
| `voice_recovery_states` | tenant/session IDs, lifecycle status, typed fault, bounded attempt, recovery deadline, safe customer message, optimistic version | one row per tenant/session; composite session FK; forced RLS | Same as session; terminal state survives service/cache restart |
| `voice_consents` | UUIDv7 consent ID, tenant/actor/session IDs, policy version, approved media region, processing/recording flags, grant/expiry/revocation timestamps | one active idempotent record per actor/session; forced RLS; database check requires processing and prohibits raw recording | Eight-hour maximum active grant; revoke on end; cascade on approved session erasure |
