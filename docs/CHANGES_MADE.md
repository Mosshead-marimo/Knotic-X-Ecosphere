...
| 7 | Interactive frontend design (Superdesign-assisted) | IN_PROGRESS |
...
### 2026-09-06 — Implemented P7-T001, P7-T002, and P7-T003 (design tokens, home page, voice call restyle)

- Phase: 7 (`P7-T001`, `P7-T002`, `P7-T003`)
- Files: `frontend/package.json`, `frontend/postcss.config.mjs` (new), `frontend/src/app/globals.css` (new), `frontend/src/app/layout.tsx`, `frontend/src/app/page.tsx`, `frontend/src/app/next.config.ts`, `frontend/src/features/voice/VoiceCallPanel.tsx`, `frontend/src/app/call/[sessionId]/page.tsx`, `docs/CHANGES_MADE.md`.
- Summary: `P7-T001` — added Tailwind CSS v4 (`@tailwindcss/postcss`, `postcss`) and `lucide-react` as real dependencies (the app had none before), and codified the Superdesign-extracted brand palette (source: ElevenLabs, via `.superdesign/brand_styles/elevenlabs.io/brand-guide.json`) as a single `@theme` token block in `globals.css` (`brand-bg`, `brand-primary`, `brand-secondary`, `brand-accent`, `brand-muted`, `brand-surface`, `brand-border`) plus `next/font/google` Inter loading in `layout.tsx`. `P7-T002` — implemented `frontend/src/app/page.tsx` as a real page translating the approved Superdesign draft (`.superdesign/drafts/home-page.html`, project `07e99595-af98-423f-81ef-1953a00c5309`), with fabricated content removed per explicit direction: the "500+ enterprise sales teams" stat, placeholder dicebear avatars, and the 400ms/99.8% precision metrics are gone; the demo call card's badge was changed from "Live" to "Sample" (it is a static mockup transcript, not a real call); the footer's unverified "All Systems Operational" indicator was dropped; the copyright line was corrected to "© 2026 Knotic" (was "© 2024 Knotic AI Inc." — wrong year, invented legal suffix); a duplicate empty `<h3>` bug in the draft's step 3 was fixed. `P7-T003` — restyled `VoiceCallPanel` and the call page to the same design-token set (status indicator dot, icon buttons, consent checkbox, error banner, `focus-visible` rings) without changing `useAgoraCall`'s `CallStatus` values, `STATUS_LABEL` copy, disabled/aria logic, or any state-machine behavior from Phase 4.
- Verification: `npm run lint --workspace @knotic/frontend` -> passed; `npm run typecheck --workspace @knotic/frontend` -> passed; `npm run build --workspace @knotic/frontend` -> passed (see build-tooling note below) with static prerender of `/` and `/_not-found` and dynamic render of `/api/health` and `/call/[sessionId]`.
- Build-tooling note: Next.js 16's default Turbopack builder cannot load `@tailwindcss/postcss`'s native `lightningcss` addon in this npm-workspaces monorepo (`Error: non-ecmascript placeable asset` — a known Turbopack limitation with native `.node` addons inside PostCSS-transform chunks, not a dependency or install defect; plain Node resolves the same module fine). `frontend/package.json`'s `dev`/`build` scripts were changed to `next dev --webpack` / `next build --webpack` to use the classic webpack compiler instead, which handles this combination correctly. `frontend/next.config.ts` also sets `turbopack.root` to the monorepo root in case Turbopack is used directly in the future.
- Follow-up: `P7-T002`'s acceptance criterion calling for a Lighthouse/accessibility audit and stakeholder sign-off, and `P7-T003`'s Phase 4 browser/accessibility/device regression matrix, have not been run yet — do before marking either task's checkbox complete in `PHASE_7_FRONTEND_DESIGN.md`. `P7-T004` (motion/interaction polish) and `P7-T005` (accessibility/responsive certification) have not started.

### 2026-09-06 — Added a development-only auth bootstrap and wired the home page to a real Agora call

- Phase: 7 (supports `P7-T002`/`P7-T003` being exercisable end-to-end; also closes a pre-existing gap Phase 0/1 never covered)
- Files: `backend/src/knotic_api/dev_auth_api.py` (new), `backend/src/knotic_api/app.py`, `backend/tests/test_dev_auth_api.py` (new), `frontend/src/features/voice/startDemoCall.ts` (new), `frontend/src/features/voice/StartDemoCallButton.tsx` (new), `frontend/src/app/page.tsx`, this file.
- Summary: No endpoint anywhere in this codebase ever issued the `knotic_session` browser-session cookie that every authenticated endpoint (`lifecycle_api`, `voice_api`) already correctly requires -- `RedisBrowserSessionStore.put()` existed but was never called from an HTTP route. Added `POST /api/v1/auth/dev-session`, registered only when the resolved environment is `development` or `test` (never staging/production), which mints a session cookie/CSRF token for a single fixed local demo tenant, idempotently ensuring that tenant's row exists (`sales_sessions.tenant_id` has a foreign key to `tenants`; no `actors` row is required since `domain_events.actor_id` carries no such foreign key). Wired the home page's "Start a demo call" and "Talk to the demo" CTAs to call this bootstrap, then create a real sales session via the existing `POST /api/v1/sessions`, then navigate to `/call/{sessionId}?csrfToken=...` -- from there the already-implemented `useAgoraCall` hook takes over unchanged (consent, real Agora token issuance, join, mute, reconnect).
- Verification: `uv run ruff format --check backend` -> `115 files already formatted`; `uv run ruff check backend` -> passed; `uv run mypy backend/src` -> `58 source files`, no issues; `npm run lint --workspace @knotic/frontend` -> passed; `npm run typecheck --workspace @knotic/frontend` -> passed; `npm run build --workspace @knotic/frontend` -> passed. `backend/tests/test_dev_auth_api.py` (`@pytest.mark.integration`, requires `KNOTIC_TEST_DATABASE_URL`/`KNOTIC_TEST_REDIS_URL`) was added but not run in this environment -- no local Postgres/Redis were available to this session.
- Follow-up: This bootstrap is explicitly a development-only placeholder for the real identity provider Phase 0/1 never built; it must never be reachable in staging or production (enforced by the environment gate in `app.py`, not yet independently verified against a deployed staging config). The full local run (real Agora App ID/Certificate, OpenAI key, Postgres, Redis, and optionally the MCP service for grounded agent responses) has not yet been executed end-to-end in this environment -- the user has real provider credentials and is expected to run it locally and report back.

### 2026-09-06 — Replaced the landing page with the VoxSales unified control center

- Phase: 7 (frontend design continuation; no phase task marked complete)
- Status: Changed
- Files: `frontend/src/app/page.tsx`, `frontend/src/app/layout.tsx`, `frontend/src/app/globals.css`, `frontend/src/features/control-center/OpsConsole.tsx`, `frontend/src/app/call/[sessionId]/page.tsx`, `frontend/src/features/voice/VoiceCallPanel.tsx`, `frontend/next.config.ts`, `docs/CHANGES_MADE.md`
- Summary: Removed the existing marketing homepage and replaced it with the responsive VoxSales AI operations console from Superdesign project `8b7746cb-8137-4a0e-a8c6-97b4301e84e5`. Refined draft `490d7546-98c8-4e0c-83ed-3bdf97bf155b` in place to version 2 (`VoxSales | AI Ops Console`; 7 generation credits). The implementation includes responsive navigation, a selectable sample-session queue, transcript and decision trace, qualification context, explicit pending/unavailable integration states, reconciliation visibility, a safe handoff review dialog, keyboard-visible controls, and reduced-motion handling. Sample content is labeled and no provider action is represented as confirmed. The production Agora call state machine and errors remain unchanged; only its visual shell was moved into the new token system. Next.js automatic agent-rule file generation is disabled so local dev does not create untracked repository policy files.
- Superdesign: [Canvas](https://superdesign.dev/teams/b54fcaee-c29a-4f01-9e08-35d8d2c01a36/projects/8b7746cb-8137-4a0e-a8c6-97b4301e84e5) · [Draft preview](https://p.superdesign.dev/draft/490d7546-98c8-4e0c-83ed-3bdf97bf155b)
- Verification: `npm run lint --workspace @knotic/frontend` passed; `npm run typecheck --workspace @knotic/frontend` passed; `npm run build --workspace @knotic/frontend` passed with `/` statically prerendered and `/call/[sessionId]` dynamically rendered. Embedded-browser verification loaded `/` with HTTP 200, rendered all key regions without a framework overlay, switched the selected session and corresponding transcript data, and opened the handoff review dialog. Desktop visual inspection passed. The required Node 24.19.0/npm 11.17.0 toolchain was used.
- Follow-up: Live queue, transcript, qualification, integration, and reconciliation feeds still require a documented backend projection API; until then the console intentionally identifies those records as sample data and disables provider-changing controls. Complete the Phase 7 mobile/cross-browser/Lighthouse accessibility matrix before marking `P7-T002` or `P7-T005` complete.
...
- Commits: `73089e5` auth/gateway; `adfb221` console projections; `a73e483` realtime stream; `40fdc34` protected routes; `0a67c7c` knowledge administration; `532616d` managed voice; `fe2288d` acceptance coverage; `b36b401` formatting; `0aba166` contracts/architecture; `71dbafb` migration-aware deployment and gateway fix.
...
- Verified: frontend ESLint and TypeScript passed with Node `24.19.0`; local and containerized Next.js `16.3.1` production webpack builds passed and generated all console/call/gateway routes; focused managed-agent/config suite `8 passed`; PostgreSQL/Redis console, SSE, knowledge, and voice integration suite `10 passed`; full backend suite completed without failures with integration-only cases skipped when service URLs were absent; Ruff passed across 164 files and mypy passed across 86 source files; API/data/MCP/operations validators and both secret scans passed. Compose rebuilt successfully, the one-shot migration exited `0`, all long-running services became healthy, SSE returned its `ready` event, and browser checks rendered `/console/live`, sessions, performance, knowledge, integrations, and system with authenticated role-aware navigation and no framework error overlay. Desktop visual inspection matched the existing Superdesign language.
...
### 2026-09-06 — Implemented the private LLM bridge, Agora webhook receiver, and managed-agent cleanup worker

- Scope: closes three of the gaps named in the previous entry (LangGraph streaming endpoint, authenticated/deduplicated Agora callbacks, inactivity/orphan cleanup), built on top of the existing `voice/managed_agent.py` / `voice_api.py` managed-agent start/stop rather than alongside a competing implementation.
- Files: `backend/src/knotic_api/voice/agent_llm.py` (new — `POST /api/v1/internal/voice/chat/completions`, the OpenAI-compatible endpoint Agora's managed agent calls for every conversational turn); `backend/src/knotic_api/agent_webhook.py` (new — `POST /api/v1/webhooks/agora`, HMAC-verified, deduplicated by delivery id); `backend/src/knotic_api/voice/agent_cleanup.py` (new — `knotic-agent-cleanup` console script, stops agents whose session ended or that exceeded a 2-hour ceiling); `backend/src/knotic_api/voice/managed_agent.py` (extended — session-binding sign/verify so the LLM bridge can recover which tenant/session a turn belongs to without trusting anything in the request body, plus an index set so the cleanup worker can enumerate active agents); `backend/src/knotic_api/config.py` (added `agora_webhook_signing_secret` and `managed_agent_configured`); `backend/src/knotic_api/app.py` (wires the LLM bridge and webhook receiver only when their settings are present); `compose.yaml` (added `${VAR:-}`-interpolated Agora/OpenAI credentials to the `backend` service only — never `frontend` or `mcp`); `.env.example`, `.gitignore`, `backend/pyproject.toml`.
- Known gaps carried forward, not silently fixed: the LLM bridge does not yet persist the customer/assistant turn to the durable `messages` table (the governed workflow checkpoint itself is durably persisted); the webhook receiver acknowledges fast but does not yet enqueue real async event persistence; the exact Agora webhook signature header/scheme has not been confirmed against a live Console configuration; TTS vendor parameter names have not been verified against Agora's current docs; no real Agora/OpenAI staging call has been run in this environment.
- Verified in this environment: `ruff format --check backend`, `ruff check backend`, and `mypy backend/src` all pass. Not yet verified: a real Docker Compose run with live Agora/OpenAI credentials, or an actual end-to-end call.
