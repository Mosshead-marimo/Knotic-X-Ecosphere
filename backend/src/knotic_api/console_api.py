...
from knotic_api.security import AuthenticatedActor, BrowserSessionValue, RedisBrowserSessionStore, RedisRateLimiter, SecurityDependencyUnavailable, derive_key
...
class ConsoleApi:
    ...
    def register(self) -> None:
        app = self.app
        app.add_url_rule("/api/v1/console/sessions", endpoint="console_sessions", view_func=self.sessions, methods=["GET"])
        app.add_url_rule("/api/v1/console/sessions/<session_id>", endpoint="console_session", view_func=self.session, methods=["GET"])
        app.add_url_rule("/api/v1/console/sessions/<session_id>/handoff", endpoint="console_handoff", view_func=self.handoff, methods=["POST"])
        app.add_url_rule("/api/v1/console/analytics", endpoint="console_analytics", view_func=self.analytics, methods=["GET"])
        app.add_url_rule("/api/v1/console/integrations", endpoint="console_integrations", view_func=self.integrations, methods=["GET"])
        app.add_url_rule("/api/v1/console/system", endpoint="console_system", view_func=self.system, methods=["GET"])
        app.add_url_rule("/api/v1/console/pending-work", endpoint="console_pending_work", view_func=self.pending_work, methods=["GET"])
        app.add_url_rule("/api/v1/console/pending-work/<work_id>/retry", endpoint="console_retry_work", view_func=self.retry_work, methods=["POST"])
    ...
    def sessions(self) -> ResponseReturnValue:
        try:
            ...
            clauses = ["s.tenant_id=:tenant_id"]
            parameters: dict[str, Any] = {"tenant_id": actor.tenant_id, "limit": limit + 1}
            if status:
                clauses.append("s.status=:status")
                parameters["status"] = status
            if cursor:
                clauses.append("(s.updated_at,s.id) < (:cursor_at,:cursor_id)")
            ...
            statement = sa.text(
                ...
                "left join leads l on l.tenant_id=s.tenant_id and l.id=s.lead_id where "
                + " and ".join(clauses)
        ...
        try:
            ...
            with self._connection(actor) as connection:
                ...
                requirements = list(connection.execute(
                    sa.text(
                        "select field,value_integer,value_text,value_text_array,value_numeric,currency,confidence,version "
                        "from requirements_current where tenant_id=:tenant_id and session_id=:session_id order by field"
                    ), {"tenant_id": actor.tenant_id, "session_id": parsed}
                ).mappings())
                objections = list(connection.execute(
                    sa.text(
            ...
            try:
                with self._connection(actor) as connection:
                    session = connection.execute(
                        sa.text("select version,status from sales_sessions where tenant_id=:tenant_id and id=:id for update"),
                    ...
                    existing = connection.execute(
                        sa.text("select id,status,priority,created_at from handoffs where tenant_id=:tenant_id and request_key_hmac=:key"),
                    ...
                    connection.execute(
                        sa.text(
                            "insert into handoffs (id,tenant_id,session_id,reason,status,context_ciphertext,priority,requested_by,request_key_hmac) "
                            "values (:id,:tenant_id,:session_id,:reason,'REQUESTED',:context,:priority,:actor_id,:key)"
                        ),
                        {"id": handoff_id, "tenant_id": actor.tenant_id, "session_id": parsed, "reason": payload.reason,
                         "context": encrypted, "priority": payload.priority, "actor_id": actor.actor_id, "key": key_hmac},
                    )
                    connection.execute(
                        sa.text(
                            "insert into domain_events (id,tenant_id,session_id,event_id,event_type,event_version,sequence,occurred_at,"
                            "correlation_id,actor_id,actor_type,payload,payload_schema_version) "
                            "select :event_id,:tenant_id,:session_id,:event_id,'operation.updated',1,coalesce(max(sequence),0)+1,:at,"
                            ":event_id,:actor_id,'HUMAN_AGENT',cast(:payload as jsonb),1 from domain_events "
                            "where tenant_id=:tenant_id and session_id=:session_id"
                        ),
                        {"event_id": new_uuid7(), "tenant_id": actor.tenant_id, "session_id": parsed, "at": now,
                         "actor_id": actor.actor_id, "payload": json.dumps({"kind": "HANDOFF", "status": "REQUESTED", "handoff_id": str(handoff_id)})},
                    )
            except IntegrityError:
                return self._problem(ConsoleProblem(409, "IDEMPOTENCY_CONFLICT", "Handoff request is already being processed."))
            return jsonify(id=str(handoff_id), status="requested", priority=payload.priority, created_at=self._timestamp(now)), 202
        except ConsoleProblem as problem:
            return self._problem(problem)
        except (DBAPIError, SecurityDependencyUnavailable):
            ...
            with self._connection(actor) as connection:
                summary = connection.execute(
                    sa.text(
                        ...
                        "count(*) filter(where outcome is not null) completed,coalesce(avg(qualification_score),0) average_score,"
                        "coalesce(avg(extract(epoch from (coalesce(ended_at,timezone('utc',now()))-started_at))),0) average_duration_seconds "
                ...
                series = list(connection.execute(
                    sa.text(
                        ...
                        "from sales_sessions where tenant_id=:tenant_id and created_at>=:since group by bucket order by bucket"
                    ), {"tenant_id": actor.tenant_id, "since": since}
                ).mappings())
            total = int(summary["total"])
            return jsonify(
                range={"days": days, "from": self._timestamp(since), "to": self._timestamp(datetime.now(UTC))},
                totals={"sessions": total, "active": int(summary["active"]), "completed": int(summary["completed"]),
                ...
                conversion_rate=round(sum(int(row["count"]) for row in outcomes if row["outcome"] in {"LEAD_QUALIFIED", "ENTERPRISE_DEMO_BOOKED"}) / total * 100, 1) if total else 0,
                outcomes=[dict(row) for row in outcomes],
                series=[{"at": self._timestamp(row["bucket"]), "sessions": row["total"], "converted": row["converted"]} for row in series],
            )
        except ConsoleProblem as problem:
            return self._problem(problem)
        except (DBAPIError, SecurityDependencyUnavailable):
        ...
        try:
            ...
            with self._connection(actor) as connection:
                rows = list(connection.execute(
                    sa.text(
                        "select provider,status,count(*) count,max(updated_at) last_activity_at from pending_provider_updates "
            ...
            for row in rows:
                item = providers.setdefault(str(row["provider"]), {"provider": row["provider"], "counts": {}, "last_activity_at": None})
    ...
    def pending_work(self) -> ResponseReturnValue:
        try:
            ...
            with self._connection(actor) as connection:
                rows = list(connection.execute(
                    sa.text(
                        "select id,provider,action,aggregate_type,aggregate_id,status,attempt_count,next_attempt_at,safe_error_code,updated_at "
                        "from pending_provider_updates where tenant_id=:tenant_id order by updated_at desc,id desc limit 100"
                    ), {"tenant_id": actor.tenant_id}
                ).mappings())
    ...
    def retry_work(self, work_id: str) -> ResponseReturnValue:
        try:
            ...
            with self._connection(actor) as connection:
                row = connection.execute(
                    sa.text(
                        "update pending_provider_updates set status='PENDING',next_attempt_at=timezone('utc',now()),"
                        "lease_owner=null,lease_expires_at=null,updated_at=timezone('utc',now()) "
                        "where tenant_id=:tenant_id and id=:id and status in ('FAILED_RETRYABLE','DEAD_LETTER') returning id,status,updated_at"
                    ), {"tenant_id": actor.tenant_id, "id": parsed}
                ).mappings().one_or_none()
            if row is None:
    ...
    def _authorize(self, *, action: str, mutation: bool = False, roles: frozenset[str] = _OPERATOR_ROLES) -> tuple[AuthenticatedActor, BrowserSessionValue]:
    ...
    def _connection(self, actor: AuthenticatedActor):
        connection = self.dependencies.engine.begin()
        class Context:
            def __enter__(inner_self):
                inner_self.connection = connection.__enter__()
                inner_self.connection.execute(sa.text("select set_config('app.tenant_id',:tenant,true)"), {"tenant": str(actor.tenant_id)})
                inner_self.connection.execute(sa.text("select set_config('app.actor_id',:actor,true)"), {"actor": str(actor.actor_id)})
                return inner_self.connection
            def __exit__(inner_self, *args: Any):
    ...
    def _session_summary(self, row: RowMapping) -> dict[str, Any]:
        ...
        return {"session_id": str(row["id"]), "version": row["version"], "status": row["status"],
                "customer": {"name": name or "Anonymous prospect", "company": company},
                "current_intent": row["current_intent"], "buying_stage": row["buying_stage"] or "NURTURE",
                "qualification_score": row["qualification_score"] or 0, "next_best_action": row["next_best_action"],
                "outcome": row["outcome"], "started_at": self._timestamp(row["started_at"]),
                "updated_at": self._timestamp(row["updated_at"]), "ended_at": self._timestamp(row["ended_at"]) if row["ended_at"] else None}

    def _message(self, tenant_id: UUID, row: RowMapping) -> dict[str, Any]:
        return {"id": str(row["id"]), "speaker": row["speaker"], "source": row["source"],
                "content": self._decrypt(row["content_ciphertext"], tenant_id, row["id"], "message_content") or "[Content unavailable]",
                "interrupted": row["interrupted"], "created_at": self._timestamp(row["created_at"])}

    @staticmethod
    def _requirement(row: RowMapping) -> dict[str, Any]:
        value = next((row[key] for key in ("value_integer", "value_text", "value_text_array", "value_numeric") if row[key] is not None), None)
        return {"field": row["field"], "value": value, "currency": row["currency"], "confidence": float(row["confidence"]), "version": row["version"]}

    def _objection(self, tenant_id: UUID, row: RowMapping) -> dict[str, Any]:
        return {"id": str(row["id"]), "category": row["category"], "status": row["status"],
                "detail": self._decrypt(row["detail_ciphertext"], tenant_id, row["id"], "objection_detail") or "Details unavailable",
                "version": row["version"]}
    ...
    def _session_operations(connection: Any, tenant_id: UUID, session_id: UUID) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for name, query in {
            "meetings": "select id,status,provider,scheduled_at,created_at from meetings where tenant_id=:tenant_id and session_id=:session_id order by created_at desc",
            "followups": "select id,status,channel,scheduled_at,created_at from followups where tenant_id=:tenant_id and session_id=:session_id order by created_at desc",
            "handoffs": "select id,status,reason,priority,assigned_agent_id,created_at from handoffs where tenant_id=:tenant_id and session_id=:session_id order by created_at desc",
        }.items():
    ...
    def _json_value(value: Any) -> Any:
        ...
        if isinstance(value, datetime): return ConsoleApi._timestamp(value)
        if isinstance(value, Decimal): return float(value)
        return value

    @staticmethod
    def _handoff_resource(row: RowMapping) -> dict[str, Any]:
        return {"id": str(row["id"]), "status": str(row["status"]).lower(), "priority": row["priority"], "created_at": ConsoleApi._timestamp(row["created_at"])}
    ...
    def _uuid7(value: str) -> UUID:
        ...
        except ValueError as error: raise ConsoleProblem(404, "RESOURCE_NOT_FOUND", "Resource was not found.") from error
        if parsed.version != 7: raise ConsoleProblem(404, "RESOURCE_NOT_FOUND", "Resource was not found.")
        return parsed

    @staticmethod
    def _integer_arg(name: str, default: int, minimum: int, maximum: int) -> int:
        try: value = int(request.args.get(name, default))
        except ValueError as error: raise ConsoleProblem(422, "VALIDATION_FAILED", f"{name} must be an integer.") from error
        if not minimum <= value <= maximum: raise ConsoleProblem(422, "VALIDATION_FAILED", f"{name} is out of range.")
        return value

    @staticmethod
    def _json(model: type[BaseModel]) -> Any:
        try: return model.model_validate(request.get_json())
        except (ValidationError, TypeError) as error: raise ConsoleProblem(422, "VALIDATION_FAILED", "Request body is invalid.") from error

    @staticmethod
    def _idempotency_header() -> str:
        value = request.headers.get("Idempotency-Key", "")
        if not 16 <= len(value) <= 128 or not value.isascii(): raise ConsoleProblem(400, "IDEMPOTENCY_KEY_REQUIRED", "A valid Idempotency-Key is required.")
        return value
    ...
    def _decode_cursor(value: str | None) -> tuple[datetime, UUID] | None:
        ...
        except Exception as error: raise ConsoleProblem(422, "VALIDATION_FAILED", "Cursor is invalid.") from error
